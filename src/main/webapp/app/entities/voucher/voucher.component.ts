...
export class VoucherComponent implements OnInit, OnDestroy {
  ...
  trackId(index: number, item: IVoucher) {
    return item.id;
  }

  registerChangeInVouchers() {
    this.eventSubscriber = this.eventManager.subscribe('voucherListModification', response => this.loadAll());
  }
